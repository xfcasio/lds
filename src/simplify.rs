    ...
    pub fn simplify(&self) -> Self {
        ...
        match self {
            ...
            Scale { coefficient, term } => match &**term {
                ...
                Scale { coefficient: coefficient2, term: term2 } => Scale { coefficient: coefficient * coefficient2, term: term2.clone() }.simplify(),

                /* distributivity */
                Sum(t1, t2) => Sum(
                    Box::new(Scale { coefficient: *coefficient, term: Box::new(t1.simplify()) }.simplify()),
                    Box::new(Scale { coefficient: *coefficient, term: Box::new(t2.simplify()) }.simplify())
                ).simplify(),

                _ => Scale { coefficient: *coefficient, term: Box::new(term.simplify()) },
            },
            ...
            Derivative { order, wrt, term } => {
                ...
                match term {
                    ...
                    _ => Derivative { order: order.clone(), wrt: wrt.clone(), term: Box::new(term.clone()) }
                }
            },
        }
    }
    ...
    pub fn debug_simplify(&self) -> Self {
        fn recursive_simplify(__self: &Term, depth: usize) -> Term {
            ...
            let simplified = match __self {
                ...
                Scale { coefficient, term } => match &**term {
                    Constant(c) => Constant(coefficient * c),
                    Var(_) | Sin(_) | Cos(_) | Power { .. } | Exponential(_, _) | Derivative { .. } => __self.clone(),
                    Scale { coefficient: coefficient2, term: term2 } => recursive_simplify(
                        &Scale { coefficient: coefficient * coefficient2, term: term2.clone() },
                        depth + 1
                    ),
                    ...
                    Sum(t1, t2) => {
                        ...
                        let s1_simplified = Box::new(recursive_simplify(
                            &Scale { coefficient: *coefficient, term: Box::new(t1_simplified) },
                            depth + 1
                        ));

                        let s2_simplified = Box::new(recursive_simplify(
                            &Scale { coefficient: *coefficient, term: Box::new(t2_simplified) },
                            depth + 1
                        ));
                    ...
                    _ => Scale { coefficient: *coefficient, term: Box::new(recursive_simplify(&term, depth + 1)) },
                },
                ...
                Sin(term) => match &**term {
                    ...
                    _ => {
                        let simplified_term = recursive_simplify(&term, depth + 1);
                        Sin(Box::new(
                            recursive_simplify(&term, depth + 1)
                        ))
                    }
                },
